...
public class printer extends halTrans{

    static {
        try {
            System.loadLibrary("Printer");
        } catch (Throwable e) {
            Log.e("jni", "i can't find printer so!");
            e.printStackTrace();
        }
    }
    ...
    public native static int PrinterWrite(byte arryData[]);

    public native static int PrinterWriteLen(byte arryData[],int len);
    ...
    public static void printBitmapGSVMSB(Bitmap bm, int bitMarginLeft, int bitMarginTop,
        ...
        printer.PrinterWriteLen(result, result.length);
